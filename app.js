...
document
  .querySelector(".select-wrapper2")
  .addEventListener("click", function () {
    this.querySelector(".select2").classList.toggle("open");
  });
...
for (const option of document.querySelectorAll(".custom-option2")) {
  option.addEventListener("click", function () {
    if (!this.classList.contains("selected")) {
      this.parentNode
        .querySelector(".custom-option2.selected")
        .classList.remove("selected");
      this.classList.add("selected");
      this.closest(".select2").querySelector(
        ".select2__trigger span"
      ).textContent = this.textContent;
    }
  });
}
